...
webhook_url = input("\033[1;32mEnter Discord Webhook URL: \033[1;33m")
...
def send_to_discord():
    try:
        with open(output_file, 'rb') as f:
            ...
            data = {'content': f'Proxy check completed. Found {len(open(output_file).readlines())} live proxies.'}
            response = requests.post(webhook_url, data=data, files=files)
            if response.status_code == 204:
                print("\033[1;32mSuccessfully sent working.txt to Discord webhook!")
            else:
...
try:
    live_count = len(open(output_file).readlines())
except FileNotFoundError:
    live_count = 0
...
subprocess.run(["python3", "ProxyV9.py"])
